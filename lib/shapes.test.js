...
describe("Circle", () => {
  ...
  describe("Square", () => {
    it("Should render a square with the given color", () => {
      ...
      const expectedOutput = '<rect x="0" y="0" width="150" height="150" fill="orange" />';
      expect(shape.render()).toContain(expectedOutput);
    });
  
    it("Should render a square with default color if color is not set", () => {
      const shape = new Square();
      const expectedOutput = '<rect x="0" y="0" width="150" height="150" fill="black" />';
      expect(shape.render()).toContain(expectedOutput);
    });
    ...
    it("Should render a square with updated color after setting", () => {
      ...
      const expectedOutput = '<rect x="0" y="0" width="150" height="150" fill="#015182" />';
      expect(shape.render()).toContain(expectedOutput);
    });
    ...
    it("Should render a square with updated text color after setting", () => {
      const shape = new Square();
      shape.setTextColor("red");
      const expectedOutput = '<text x="75" y="75" font-size="32" fill="red" text-anchor="middle" alignment-baseline="central">SVG</text>';
      expect(shape.render()).toContain(expectedOutput);
    });
    ...
    it("Should render a square with updated text after setting", () => {
      const shape = new Square();
      shape.setText("Hello");
      const expectedOutput = '<text x="75" y="75" font-size="32" fill="white" text-anchor="middle" alignment-baseline="central">Hello</text>';
      expect(shape.render()).toContain(expectedOutput);
    });
  });